...
<div class="f_control-panel">
    ...
    <div class="f_cp__bottom">
        <div class="f_cp__bottom_left">
        	...
        	<div class="f_cp_bl_action-buttons">
            	<a href="{{ route('grado.crear') }}" class="f_button f_button--primary tight">NUEVO</a>
        	</div>
            {{-- /Acciones buttons --}}
        </div>
        <div class="f_cp__bottom_right">
			...
			<div class="f_cp__pager">
				<span>
					{{ $pgdGradosLimits['from'] . ' - '.  $pgdGradosLimits['to']  }} / {{ $pgdGrados->total() }}
				</span>
				@if ($pgdGrados->hasPages())
				<span>
                    {{-- Previous Page Link --}}
                    @if (!$pgdGrados->onFirstPage())
					<a rel="prev" href="{{ $pgdGrados->previousPageUrl() }}"
                    ...
                    {{-- Next Page Link --}}
                    @if ($pgdGrados->hasMorePages())
                    <a rel="next" href="{{ $pgdGrados->nextPageUrl() }}"
                    	class="f_button f_button--secondary tight" title="Página Anterior">
	...
	<div class="f_filters">
		<div class="f_filters__hidden-panel" id="filtersHiddenPanel">
        	...
        	<form action="{{ route('grado.lista') }}" method="get" 
        			class="f_filters__options-panel f_input-box--flat"
        			id="formFilters">
    			<div class="f_filters__title">Filtrar grados</div>
...
<div class="f_card-section">
    ...
    <div class="f_card-section__body">
        ...
        <div class="f_overflow_scroll-x">
            <table class="f_table" id="tableGrados">
                <thead>
                    <tr class="list_title">
                ...
                <tbody>
                	@foreach($pgdGrados as $grado)
                    <tr class="f_oddeven">
                        <td>
                            <a href="{{ route('grado.mostrar', ['id' => $grado]) }}" class="f_link" style="display: flex">
                                <span style=" color: #a69944">
                                    <i class="fas fa-graduation-cap mr-1"></i>
                                </span>
                                <span class="align-middtle">{{ $grado->GRD_CODIGO }}</span>
                            </a>
                        </td>
                        <td class="f_nowrap_200"> {{ ucfirst($grado->GRD_NOMBRE) }}</td>
                        <td class="f_nowrap_200">{{ $grado->GRD_DESCRIPCION }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
